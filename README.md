...
This repository contains Helm charts and **ArgoCD GitOps** configuration for Fancia on EKS.

### ArgoCD GitOps (CD)

Deployments are managed by **ArgoCD** (pull-based GitOps), not by `helm upgrade` in CodeBuild.

| Component | Location |
|---|---|
| ArgoCD Application chart | `argocd/templates/{app,platform}/` |
| ArgoCD server install values | `argocd-values.yaml` + `argocd-values-{env}.yaml` |
| App workloads Helm chart | `main-chart/` |
| Dynamic Helm values | `*/values-{env}.yaml` (generated by `values-gen.py`) |
| Static Helm values | `*/values.yaml` (manual / app CI) |
| Bootstrap | `buildspec_argocd_bootstrap.yaml` via infra-pipeline |

**Infra pipeline flow:** Terraform Apply → `ArgoCD_Bootstrap` → `values-gen.py` → `helm template argocd | kubectl apply` → root Application syncs from Git.

**App deploy flow:** CodeBuild pushes image to ECR → updates `services.{name}.imageVersion` in `main-chart/values.yaml` → commit → ArgoCD syncs.

### Three Helm layers

| Layer | Purpose |
|---|---|
| `argocd-values.yaml` | Static values for upstream `argo-cd` chart (server install) |
| `argocd/` | Helm chart rendering ArgoCD Application + AppProject CRs |
| `main-chart/` | Fancia microservice workloads |

Use `scripts/bump-image-version.sh` from backend CI after ECR push
...
4. Execute `values-gen.py` to generate dynamic value files (`values-{env}.yaml`):

   ```bash
   ENVIRONMENT=dev PROJECT_NAME=fancia GITHUB_USERNAME=you python3 values-gen.py --out-dir=.
   helm template argocd ./argocd -f argocd/values.yaml -f argocd/values-dev.yaml
   ```

5. Deploy the Helm charts (local bootstrap without ArgoCD):

    ```bash
    ENVIRONMENT=dev PROJECT_NAME=fancia python3 values-gen.py --out-dir=.
    helm upgrade --install "$NAMESPACE" ./main-chart \
      --namespace "$NAMESPACE" \
      --create-namespace \
      -f main-chart/values.yaml \
      -f main-chart/values-dev.yaml
    ```

    For GitOps deployments, prefer the infra-pipeline `ArgoCD_Bootstrap` stage instead of manual `helm upgrade`.
