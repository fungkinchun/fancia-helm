...
def main():
    ...
    keys_to_extract = ['projectName', 'awsAccountId', 'awsRegion', 'domainName', 'email', 'vpcId', 'acmCertificateArn', 'privateHostedZoneId', 'publicHostedZoneId']
    ...
    if (
        ...
        for idx, (k, v) in enumerate(tf_outputs[rds_name_map_key].items()):
            values["repositories"].append({
                ...
                "databaseSecretName": v["databaseSecretName"],
                "port": 8080 + idx,
                "imageVersion": "latest",
            })
    else:
    ...
    if (
        ...
        for secret in tf_outputs[credentials_name_map_key].keys():
            values["secrets"].append({
                "name": secret,
                "secretName": tf_outputs[credentials_name_map_key][secret]["secretName"],
                "namespace": tf_outputs[credentials_name_map_key][secret]["namespace"],
            })
    else:
